...
export default function CoupletList() {
  ...
  const fetchData = async (p: number, kw: string) => {
    setLoading(true);
    try {
      const res = await listCouplets(p, pageSize, kw);
  ...
  const columns = [
    ...
    { title: '药对名', dataIndex: 'herb_couplet_name', key: 'name',
      render: (t: string, r: HerbCoupletBasic) => <a onClick={() => navigate(`/couplets/${r.id}`)}>{t}</a> },
  ...
  return (
    <div>
      <Title level={3} className={styles.pageTitle}>毒性药对</Title>
      <div className={styles.searchBar}>
        <Input placeholder="搜索药对名、拼音、毒性..." value={keyword}
